...
[Space Invaders Anno DOMani][space-invaders-anno-domani] - a fast-paced Space Invaders JavaScript browser game that utilizes DOMani

[space-invaders-anno-domani]: https://github.com/kchen21/SpaceInvadersAnnoDOMani

DOMani is a lightweight JavaScript DOM manipulation library. It consists of a core function, $l, and a class `DOMNodeCollection`, whose instances hold onto DOM elements, allowing its methods to manipulate and traverse them.
...
If the argument is a string that has the following structure: '&lt;tag&gt;innerHTML&lt;/tag&gt;', then `$l` creates a new DOM element using it and returns a custom `DOMNodeCollection` object that holds that element for manipulation purposes. If the argument is string representing a CSS selector, `$l` returns a `DOMNodeCollection` object that holds the DOM nodes that match the selector.
